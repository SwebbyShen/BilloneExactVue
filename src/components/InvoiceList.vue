    ...
    <div id="invoicesearchinfo">
        ...
        <div class="form-group row" @keyup.enter="searchInvoices" v-show="showsearchbox">
            <div class="form-group row col-md-7">
                ...
                <div class="col-sm-4">
                    <InputNumber style="width:100%;" v-model="billonenofrom"/>
                </div>
                <div class="col-sm-4">
                    <InputNumber style="width:100%;" v-model="billonenoto"/>
                </div>
            </div>
            ...
            <div class="form-group row col-md-5 mt-2">
                ...
                <div class="col-sm-1 flex align-items-center">
                    <Dropdown v-model="finalregisterStatus" :options="registerStatus" placeholder="全て" filter optionLabel="label" style="height:43px;" @change="searchdatastatus = finalregisterStatus.label">
                        <template #option="slotProps">
            ...
            <TabPanel>
                ...
                <div class="row">
                    <DataTable :value="invoicedata" resizableColumns columnResizeMode="fit" showGridlines tableStyle="min-width: 50rem"
                        ...
                        <Column v-for="(col, index) of invoicecolumnsselected" :showFilterMenu="(col.field == 'registerstatus' ? false : true)" :field="col.field" :header="col.header" :key="col.field + '_' + index" :sortable="(col.field == 'registerstatus' ? false : true) ">
                            <template #body="{ data, field }">
                                <template v-if="field === 'registerstatus'">
                                    <button type="button" class="btn btn-warning btn-sm" v-if="data[field] == '一時登録中'" v-on:click="fncInvoiceDetail(data.invoiceurl)">一時登録中</button>
                                    <button type="button" class="btn btn-info btn-sm" v-else-if="data[field] == '事前登録'" v-on:click="fncInvoiceDetail(data.invoiceurl)">事前登録</button>
                                    <!--<button type="button" class="btn btn-success btn-sm" v-else-if="data[field] == '新規'" v-on:click="fncInvoiceDetail(data.invoiceurl)">新規</button>-->
                            ...
                            <template #filter="{ filterModel, filterCallback, field}">
                                <template v-if="field === 'registerstatus'">
                                    <Dropdown v-model="filterModel.value" @change="filterCallback()" :options="registerStatus.map(option => option.label)" placeholder="" class="p-column-filter" style="min-width: 12rem" :showClear="true">
                                    </Dropdown>
                                </template>
                                <template v-else>
...
<script>
    ...
    export default {
        setup() {           
            ...
            return {
                errortoast, filters, infotoast, statuses, showmenu, CommonSetting, helpMenu,
            };
        },
        computed: {
            filterText: {
        ...
        methods: {
            ...
            getInvoiceList() {
                ...
                axios.post('https://billoneexactapi.azurewebsites.net/api/GetInvoiceList'
                    , params).then(
                        response => (
                            this.invoicedata = JSON.parse(response.data), this.datacount = this.invoicedata.length, this.loading = ref(false)
                        )
                ).catch(error => {
                    if (JSON.stringify(error).indexOf("401") >= 0) {
                        window.location.reload();
